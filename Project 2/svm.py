...
def svc(c):
    svc_classifier = svm.LinearSVC(C=c, max_iter=4000)
    svc_classifier.fit(train_matrix, train_docs_classes)
    predict = svc_classifier.predict(test_matrix)
    accuracy = 0
...
for x in d:
    d[x] = svc(x)
    print("for c = ", x, ", accuracy: ",d[x])
best = max(d, key=d.get)
print("the best regularizer: ", best, " with accuracy ", d[best])
